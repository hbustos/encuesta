<?php

/**
 * This is the model class for table "Direccion".
 *
 * The followings are the available columns in table 'Direccion':
 * @property integer $Id
 * @property integer $IdReferencia
 * @property string $Direccion
 * @property string $Barrio
 * @property integer $Ciudad
 */
class Direccion extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return Direccion the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'Direccion';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('IdReferencia, Direccion, Ciudad', 'required'),
			array('IdReferencia, Ciudad', 'numerical', 'integerOnly'=>true),
			array('Direccion, Barrio', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('Id, IdReferencia, Direccion, Barrio, Ciudad', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'ciudad' => array(self::BELONGS_TO, 'Municipio', 'Ciudad'),
			'id' => array(self::BELONGS_TO, 'Beneficiario', 'Id'),
			'idReferencia' => array(self::BELONGS_TO, 'Referencia', 'IdReferencia'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'Id' => 'Id',
			'IdReferencia' => 'Id Referencia',
			'Direccion' => 'Direccion',
			'Barrio' => 'Barrio',
			'Ciudad' => 'Ciudad',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;
		$criteria->compare('Id',$this->Id);
		$criteria->compare('IdReferencia',$this->IdReferencia);
		$criteria->compare('Direccion',$this->Direccion,true);
		$criteria->compare('Barrio',$this->Barrio,true);
		$criteria->compare('Ciudad',$this->Ciudad);

		return new CActiveDataProvider('Direccion', array(
			'criteria'=>$criteria,
		));
	}
        public function beforeSave()
        {
                if(parent::beforeSave())
                {
                        $this->Direccion = strtoupper($this->Direccion);
                        $this->Barrio = strtoupper($this->Barrio);
                        return true;
                }
                return false;
        }

}
